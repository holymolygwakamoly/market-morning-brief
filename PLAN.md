...
| 모델 | stage1·stage2 `claude-sonnet-5` | **결정 13: 모든 단계 opus**(`BRIEF_MODEL`). stage1 예산 480s(opus로 167건 태깅이 느림), stage2 480s, 내부 데드라인 30분(러너 킬 제약 없음) |
...
| 영역 | 내용 |
|------|------|
| 상단 바(공통) | 기준일 date input(기본 오늘 KST) · [업데이트] 버튼(**기준일 == 오늘 KST일 때만 활성**) · 마지막 업데이트 시각 · 진행률(탭별 단계) · [GitHub에 게시](기존 유지) |
| 좌측 탭 | ① 홈 ② 거시경제 ③ 섹터별 주요뉴스 ④ 미국시장 ⑤ 유럽시장 ⑥ 한국시장 ⑦ 중국시장. 탭마다 생성 상태 뱃지(완료/생성 중/실패/없음) |
| ① 홈 | 지수 카드 그리드(지역별: 미국·유럽·아시아/한국·중국) — 종가·전일 대비 등락률·등락폭·asof 배지·5일 스파크라인. 하단: 환율/금리/원자재 표, **오늘의 핵심 5개**(6개 보고서에서 추린 요약, 결정 17 — 항목 클릭 시 해당 탭으로 이동), 소스 상태 요약. 카드 클릭 → 지수 상세 |
| 지수 상세(홈 하위) | **미국(S&P500·나스닥100·다우30)·한국(코스피·코스닥) 지수만**(결정 15). 서브탭 **등락폭 / 거래대금 / 시가총액** 각각 내림차순 표(종목·티커·섹터·종가·등락률·거래량·거래대금·시가총액). 상단에 상승/하락 종목 수, 상위 5 상승·하락. 유럽·중국·나스닥 종합 등 나머지 지수 카드는 클릭 시 지수 자체의 5일 차트·등락만 표시(구성종목 없음) |
| ②~⑦ 보고서 탭 | 각 주제별 상세 한국어 보고서(§11.4 구조). 하단에 근거 기사 링크·데이터 표·소스 상태 |

### 11.2 기준일과 "직전 완료 세션" 정의

- 기준일 D = KST 날짜. 사용자는 아침(또는 낮)에 D를 오늘로 두고 [업데이트].
- **직전 완료 세션**: 미국·유럽 = D 새벽 KST에 마감된 세션(현지 D−1) / 한국·중국·홍콩·일본 = D−1 영업일 세션. 주말·휴장이면 가장 최근 완료 세션(카드에 `asof` 배지). Yahoo 일봉 `range=10d`에서 **세션 날짜 < D(현지 기준)** 인 마지막 봉을 선택해 장중 값이 섞이지 않게 한다(오후에 업데이트해도 한국장 당일 장중 값을 쓰지 않음).
- 뉴스 수집 윈도: 기존 규칙 유지(직전 영업일 06:50 KST ~ 업데이트 시각, 월요일 ~72h).

### 11.3 데이터 소스 (신규 — 모두 **Step A 프로브로 검증 후 확정**)

**지수(홈 카드, Yahoo chart JSON — 기존 어댑터 확장)**
| 지역 | 심볼 |
|------|------|
| 미국 | ^GSPC S&P500 · ^IXIC 나스닥 종합 · ^NDX 나스닥100 · ^DJI 다우 · ^RUT 러셀2000 · ^VIX · ^SOX 필라델피아 반도체 |
| 유럽 | ^STOXX50E 유로스톡스50 · ^STOXX 스톡스600 · ^GDAXI DAX · ^FTSE FTSE100 · ^FCHI CAC40 |
| 한국 | ^KS11 코스피 · ^KQ11 코스닥 · (^KS200 코스피200) |
| 중국·홍콩·일본 | 000001.SS 상해종합 · 399001.SZ 심천성분 · 000300.SS CSI300 · ^HSI 항셍 · ^HSTECH 항셍테크 · ^N225 닛케이 |
| 거시 | KRW=X · DX-Y.NYB · JPY=X · EURUSD=X · CNY=X · ^TNX 미10Y · ^IRX 미3M · ^FVX 미5Y · CL=F WTI · BZ=F 브렌트 · GC=F 금 · HG=F 구리 · BTC-USD |

**구성종목 순위(지수 상세)** — 필요 필드: 티커·이름·섹터·종가·등락률·거래량·거래대금·시가총액. **결정 15: 미국·한국 지수만.**
| 지수 | 구성종목 목록 | 종목별 시세·거래량 | 시가총액 | 판정 |
|------|--------------|--------------------|----------|------|
| S&P500 · 나스닥100 · 다우30 | Wikipedia 구성종목 표(CC-BY-SA) 또는 iShares(IVV)/Invesco(QQQ) 보유내역 CSV | nasdaq.com 스크리너 JSON 1회(전 종목 등락·거래량·시총 포함) — 실패 시 Yahoo 종목별 호출(≈630회, 스로틀) | 스크리너 포함 | **채택(프로브)** |
| 나스닥 종합 | 3,000+ 종목 | — | — | 카드만(구성종목은 나스닥100으로) |
| 코스피 · 코스닥 전 종목 | KRX 정보데이터시스템 공개 JSON(전 종목 시세·거래대금·시총·업종) | 동일 | 포함 | **채택(프로브)** — KRX 이용약관 확인. 실패 시 네이버 금융 공개 페이지는 크롤링 정책(robots) 확인 후에만 |
| 유럽(DAX·FTSE·CAC·유로스톡스50·스톡스600) · 중국(CSI300·항셍·항셍테크·상해·심천) · 러셀2000 | — | — | — | **카드만(결정 15)** — 종목별 시세 수백 회 호출·시총 미제공 문제로 제외. 유럽·중국 보고서의 주도주는 뉴스 기반 서술 |

- 거래대금 = 소스가 주면 그 값, 없으면 `거래량 × 종가` 추정(표에 "추정" 표기).
- 미국 폴백(nasdaq 스크리너 실패 시) Yahoo 종목별 호출: 동시 4·간격 0.25s·429 시 60s 백오프·지수당 예산 4분. 예산 초과 시 **받은 종목만으로 표를 만들고 "N/M 종목" 배너**.

**한국 수급(한국시장 보고서용)**: KRX 투자자별 매매동향(외국인/기관/개인, 코스피·코스닥, 업종별) — 공개 JSON, 프로브 대상. 미국·유럽·중국은 무료 수급 데이터가 없으므로 **상승/하락 종목 수·거래량·섹터 ETF 등락·뉴스**로 대신 서술한다(보고서에 "수급 데이터 없음" 명시). 중국 북향자금(Stock Connect)은 HKEX 공개 페이지 프로브 후 결정.

**섹터 ETF(섹터별 주요뉴스·미국시장 데이터)**: XLK XLC XLY XLP XLE XLF XLV XLI XLB XLRE XLU · SOXX · ITA(방산) · XBI · ARKK 등 Yahoo chart.

**뉴스 소스 추가(기존 12개 + 아래, 프로브)**
| 지역 | 후보 |
|------|------|
| 유럽 | ECB 보도자료 RSS · BoE 뉴스 RSS · Google News `"European stocks" OR DAX OR "FTSE 100" OR "Euro Stoxx"`(en) · Google News `유럽 증시`(ko) |
| 중국 | Google News `China stocks OR "Hang Seng" OR CSI 300`(en) · Google News `중국 증시 OR 항셍 OR 상해종합`(ko) · Google News 中文(`A股 OR 港股`, hl=zh-CN) — LLM이 중국어 제목 해석 가능 |
| 거시 | 기존 Fed·BOK + BLS 보도자료 RSS · 미 재무부 · IMF/OECD RSS · Google News `경제지표 OR 금리 OR 환율`(ko) |
| 섹터/지정학 | Google News `defense stocks OR 방산` · `oil prices` · `semiconductor` 등 섹터 쿼리 6~8개 |
| 일본(카드용 뉴스 최소) | Google News `Nikkei`(en) |

### 11.4 보고서 6종 — 구조(각각 stage2 opus 1회, 구조화 출력)

공통: 한국어, 분량 상한 없음, "입력에 없는 수치 금지·종목은 입력 등장분만", 투자 조언 아님 고지, 근거 기사 링크. 각 보고서는 **자기 지역 데이터 표(지수·구성종목 상위·ETF·수급)를 입력으로 받아** 수치를 근거로 서술한다.

| 탭 | 섹션 |
|----|------|
| 거시경제 | 한눈에 보기(3~5줄) → 금리·채권(미 2Y/10Y, 커브, 실질금리) → 환율(DXY·원/달러·엔·유로·위안) → 원자재(유가·금·구리) → 중앙은행(Fed/ECB/BOJ/BOK/PBOC 발언·일정) → 어제 발표 지표와 해석 → 오늘·이번 주 일정 → 지정학 → 자산별 함의(주식/채권/환율) |
| 섹터별 주요뉴스 | 오늘 이슈 섹터 목록(방향·이유) → 섹터별 블록(세계 주요 뉴스 요약, 관련 시장 US/EU/KR/CN 영향, 대표주 1~5, 섹터 ETF 등락) → **AI 섹터 필수** → 지정학·전쟁·규제 등 다중 섹터 이벤트(예: 전쟁 재개 → 방산·에너지·항공) |
| 미국시장 | 지수 흐름(전일 세션 서술, 장중 변곡·마감) → 시장 폭(상승/하락 종목 수·거래량·VIX) → 섹터 ETF 등락 순위와 이유 → 주도주·급등락 종목(구성종목 순위 상위) → 실적·기업 뉴스 → 오늘 이벤트(실적·지표·연준) → 한국장 영향 |
| 유럽시장 | 지수 흐름(스톡스600·DAX·FTSE·CAC) → 섹터·주도주 → ECB/BoE·유럽 거시 → 주요 뉴스 → 한국·미국 영향 |
| 한국시장 | 코스피·코스닥 흐름 → **투자자별 수급(외국인·기관·개인, 업종별)** → 업종 등락 → 주도주·급등락(구성종목 순위) → 주요 뉴스·더벨 헤드라인 → 오늘 이벤트(공시·지표·수급 예상) → 미국 세션 반영 예상 |
| 중국시장 | 상해·심천·CSI300·항셍·항셍테크 흐름 → 북향자금(가능 시) → 정책(PBOC·국무원)·규제 → 섹터·주도주 → 주요 뉴스 → 한국 영향(중국 노출 업종) |

**홈 "오늘의 핵심 5"**: 별도 호출 없이 6개 보고서 각각의 `headlines[]`(1~2개, importance 포함)를 모아 중요도순 5개 선택.

### 11.5 파이프라인(업데이트 1회)

```
[업데이트 D] (D == 오늘 KST 검증 + 오늘 미실행 검증(결정 18: 하루 1회), 서버·클라이언트 이중)
 1. market  — Yahoo 지수·거시·섹터 ETF(≈45심볼, 병렬 8) + 구성종목(미국 3·한국 2, §11.3) + KRX 수급   ≤ 5분(병렬)
 2. collect — 뉴스 소스 ≈25개(ThreadPool, 3분 예산), 윈도·dedupe(소스당 40, 총 400)
 3. stage1  — 태깅(opus): 지역(US/EU/KR/CN/MACRO/GLOBAL)·섹터·중요도·story_key. 400건 → 2배치 병렬(호출 ≤3, 예산 8분)
 4. select  — 보고서별 쿼터(예: 미국 40 + 관련 거시 10 / 섹터 = 섹터 태그 상위 60 / 거시 40 / 유럽 30 / 한국 40 + 더벨 30 / 중국 30)
 5. stage2  — 보고서 6개, 각 opus 1회(재시도 ≤2), **동시 3개**(CLI 프로세스 3개) → 예산 보고서당 8분, 단계 전체 20분
 6. render  — docs/data/D/*.json + 정적 페이지. 보고서별 독립 실패(한 탭 실패해도 나머지 저장·표시, 탭에 실패 배지 + 재생성 버튼)
 7. status  — 소스·구성종목 커버리지·토큰·경고
 내부 데드라인 45분. 한 탭 [재생성](결정 16)은 해당 stage2만 다시 실행(market/collect 스냅샷 재사용) — 실패 탭·품질 불만 탭 대상, 당일만.
```

**예상 시간**: happy 12~20분(market 3 + collect 2 + stage1 4 + stage2 병렬 8 + render) / worst 45분. 사용자 수용(결정 16). **Max 사용량**: 1회 업데이트에 opus 출력 ≈ 60~90k 토큰(현재 ≈ 30k의 2~3배). 5시간 창에서 하루 1회는 무리 없으나 재생성 반복 시 한도 도달 가능 → 대시보드에 CLI 한도 오류를 그대로 표시.

### 11.6 저장 구조·게시·업데이트 규칙

```
docs/
  index.html                      대시보드 SPA(정적, 인라인 JS·CSS) — ?date=YYYY-MM-DD 로 스냅샷 로드
  data/index.json                 존재하는 기준일 목록(최신순) + 각 날짜 result
  data/YYYY-MM-DD/
    home.json                     지수 카드·거시 표·핵심 5·소스 상태
    indices/<symbol>.json         구성종목 순위(3기준 정렬은 클라이언트에서, 원본 1벌)
    reports/{macro,sectors,us,eu,kr,cn}.json   보고서 본문(구조화) + 근거 링크
    status.json
  reports/YYYY-MM-DD.html         (기존 단일 보고서 형식은 폐기, 폴더는 아카이브로 유지)
```
- **정적 SPA + 로컬 API 분리**: 페이지 자체는 정적 파일이라 로컬(`brief.serve`)과 GitHub Pages에서 동일하게 동작. [업데이트]/[재생성]/[게시] 버튼만 로컬 서버(`/api/*`)가 필요하며 Pages에서는 숨김.
- **업데이트 규칙(결정 18: 하루 1회)**: `POST /api/update {date}` — `date != 오늘 KST` → 400. 오늘 `status.json`이 이미 `success|degraded`면 409 + 버튼 비활성("오늘 업데이트 완료 HH:MM"). `failed`(전체 실패)면 재실행 허용. 탭별 `POST /api/regenerate {tab}`는 당일에 한해 횟수 제한 없음. 실행 중 409.
- **과거 기준일**: `data/index.json`에 있으면 로드, 없으면 "YYYY-MM-DD 업데이트 내역 없음"(생성 불가 안내). 기존 "오늘~3일 전 생성" 기능은 **폐기**.
- 게시: `git add docs && commit && pull --rebase && push`(기존 유지). 스냅샷 1일 ≈ 구성종목 JSON 포함 2~4MB → 리포 증가량 월 ≈ 60~90MB. 필요 시 구성종목 JSON은 상위 300종목으로 절단(설정).

### 11.7 구현 단계(승인 후 ralph)

| Step | 내용 | 완료 기준 |
|------|------|-----------|
| A 프로브 | §11.3 신규 소스 전부 `scripts/probe.py` 확장: 구성종목(nasdaq 스크리너·KRX·iShares CSV·Yahoo 종목별 429 여부)·KRX 수급·신규 RSS/Google News 쿼리. 3회 반복 | 소스 표 "판정" 열 확정, 미지원 지수 목록 확정 |
| B 시장 데이터 계층 | `brief/market/`: indices(세션 선택 규칙)·constituents(어댑터별)·krx_flows·sector_etf. 캐시·스로틀·부분 결과 | 단위 테스트(세션 선택·부분 결과·429 백오프), `--dump`로 D 스냅샷 생성 |
| C 뉴스·태깅 확장 | sources.yaml 지역 확장, stage1 스키마에 region 추가, 2배치 병렬, 보고서별 select 쿼터 | 테스트, 배치 캡 ≤3 |
| D 보고서 6종 | 스키마 6개(검증용 validator 포함: AI 섹터 ≥300자 등 기존 규칙 승계), 프롬프트 6개, stage2 병렬 3, 탭별 독립 실패 | 모킹 테스트 + 실제 1회 |
| E 대시보드 SPA·API | index.html SPA(좌측 탭·기준일·카드·지수 상세 3정렬·보고서 뷰·상태), `/api/update`·`/api/regenerate {tab}`·`/api/status`, 당일 검증 | 브라우저 확인(데스크톱·모바일), test_serve |
| F 전환·문서 | 기존 단일 보고서 경로 제거, README·HANDOFF·PLAN 갱신, 첫 실제 업데이트 관측(시간·사용량·구성종목 커버리지) | 사용자 확인 |

### 11.8 리스크

| 위험 | 완화 |
|------|------|
| 구성종목 시세(미국 폴백 Yahoo 630회) 429 | nasdaq 스크리너 1회가 주 경로. 폴백 시 스로틀·예산·부분 표 + 배너 |
| nasdaq.com/KRX 비공식 JSON 변경·차단 | 프로브 게이트, 이용약관 확인 후 README §4에 근거 기록, 실패 시 Yahoo 종목별 폴백 |
| 업데이트 30분+ 및 Max 한도 | 병렬 3, 탭별 재생성, 한도 오류 표시. 하루 1회 규칙으로 사용량 상한 |
| 유럽·중국 뉴스 얕음(무료 RSS 부족) | Google News 쿼리 다중화, 중국어 소스 허용, 보고서에 소스 한계 명시 |
| 스냅샷 용량으로 리포 비대 | 구성종목 절단 옵션, 오래된 스냅샷 정리 스크립트(follow-up) |
| 장중 업데이트 시 세션 혼입 | §11.2 세션 선택 규칙(세션 날짜 < D) + asof 배지 |

### 11.9 확정 답변(결정 15~18, 2026-09-17)

| # | 질문 | 답변 | 반영 |
|---|------|------|------|
| 15 | 구성종목 순위 범위 | **미국·한국만 구성종목** | 유럽·중국·나스닥 종합·러셀·스톡스600은 카드만. §11.1·11.3 |
| 16 | 업데이트 방식 | **한 번에 전부(병렬 3) + 탭별 재생성** | §11.5, `/api/regenerate {tab}` |
